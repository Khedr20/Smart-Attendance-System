...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["password"]))){
        $password_err = "<p class='error'>Please enter a password.</p>";    
        echo $password_err; 
    } elseif(strlen(trim($_POST["password"])) < 6){
        $password_err = "<p class='error'>Password must have atleast 6 characters.</p>";
        echo $password_err;
    } else{
    ...
    if(empty(trim($_POST["confirm_password"]))){
        $confirm_password_err = "<p class='error'>Please confirm password.</p>";     
        echo $confirm_password_err;
    } else{
        $confirm_password = trim($_POST["confirm_password"]);
        if(empty($password_err) && ($password != $confirm_password)){
            $confirm_password_err = "<p class='error'>Password did not match.</p>";
            echo $confirm_password_err;
        }
    }
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    ...
                    if(mysqli_stmt_fetch($stmt)){
                        if(empty($password_hash)){
                            ...
                            echo "<p class='error'>You already have set a password, please contact the admin if you want to change your password.</p>";
                        }
                    }
                }
            }
...
<main>
 ...
 <input type="password" name="confirm_password" id="confirm-password" placeholder="Confirm Password"><br><br>
